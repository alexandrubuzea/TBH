...
export async function tryV2Migration(config) {
    if (process.env["CI"] === "true")
        throw new UserError(`You are using an old version of the YAML configuration file. Please update it to the latest version. For more information, check the migration guide at https://genezio.com/docs/learn-more/upgrading-to-v1`);
    try {
        const v1Config = await v1.create(config);
        const { migrate } = await inquirer.prompt({
            name: "migrate",
            type: "confirm",
            message: "Your project configuration is using an old version of the YAML configuration file. Would you like to migrate it to the latest version?",
        });
        if (!migrate) {
            throw new UserError("genezio >= 1.0.0 needs a `genezio.yaml` file with version 2. For more information, check the migration guide at https://genezio.com/docs/learn-more/upgrading-to-v1");
        }
        let frontendPath = undefined, frontendPublish = undefined;
        ...
        if (v1Config.cloudProvider ===
            CloudProviderIdentifier.CLUSTER) {
            throw new UserError("genezio >= 1.0.0 is required for the migration of the cloud provider to cluster");
        }
        const v2Config = {
            name: v1Config.name,
