...
import currentGenezioVersion, { checkNodeMinimumVersion, logOutdatedVersion, } from "./utils/version.js";
...
program
    .addOption(new Option("--logLevel <log-level>", "Show debug logs to console.").choices([
    "trace",
    "debug",
    "info",
    "warn",
...
program
    ...
    .description(`${colors.green("genezio v" + currentGenezioVersion)} Build and deploy applications to the genezio infrastructure.`)
    .exitOverride((err) => {
...
program
    ...
    .option("--subdomain <subdomain>", "Set a subdomain for your frontend. If not set, the subdomain will be randomly generated.", undefined)
    .option("--config <config>", "Use a specific `genezio.yaml` file as deployment configuration", "./genezio.yaml")
    .summary("Deploy your project to the cloud.")
    .description(`Use --frontend to deploy only the frontend application.
...
program
    ...
    .option("-s, --stage <stage>", "Set the stage on which you want to test your services on", "prod")
...
create
    ...
    .addOption(new Option("--region <region>", "Region of the project.").choices(regions.map((region) => region.value)))
    .addOption(new Option("--backend <backend>", "Backend template.").choices(Object.keys(backendTemplates)))
    .addOption(new Option("--frontend <frontend>", "Frontend template.").choices(Object.keys(frontendTemplates)))
    .option("--multirepo", "Create a project with a backend and a frontend in separate repositories.", false)
    .option("--path <path>", "Path where to create the project.", undefined)
...
program
    ...
    .action(async (classPath, classType) => {
...
program
    ...
    .addOption(new Option("--source <source>", "Where the SDK should be generated from.")
    .choices(["local", "remote"])
    .default("remote"))
    .option("-c, --config <config>", "Path to the genezio.yaml file on your disk. Used for loading project details from a genezio.yaml file, instead of command arguments like --name", "./")
    .option("-o, --output <output>", "Path to the directory where the SDK will be generated.", "./sdk")
...
program
    ...
    .description("Linking a client with a deployed project will enable `genezio local` to figure out where to generate the SDK to call the backend methods.\n\
...
program
    ...
    .argument("[projectName]", "The name of the project that you want to communicate with. If --all is used, this option is ignored.")
    .summary("Unlink the generated SDK from a client.")
    .description("Clear the previously set path for your frontend app, which is useful when managing a project with multiple repositories.\n\
