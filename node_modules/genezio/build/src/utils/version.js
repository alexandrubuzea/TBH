...
export async function logOutdatedVersion() {
    ...
    if (cmp(latestGenezioVersion, ">", currentGenezioVersion)) {
        log.info(boxen(`Update available ${colors.grey(currentGenezioVersion)} → ${colors.magenta(latestGenezioVersion)}\nRun ${colors.green("npm i -g genezio")} to update`, {
            padding: 1,
            margin: 1,
            borderStyle: "round",
...
export function checkNodeMinimumVersion() {
    if (cmp(process.version, "<", NODE_MINIMUM_VERSION)) {
        throw new UserError(`Genezio CLI requires Node.js version v${NODE_MINIMUM_VERSION} or higher. You are currently running Node.js ${process.version}. Please update your version of Node.js.`);
    }
}
