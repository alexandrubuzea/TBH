...
async function tryToReadClassInformationFromDecorators(yamlBackend) {
    ...
    const allFilesPaths = (await getAllFilesFromPath(cwd)).filter(decoratorExtractor.fileFilter(cwd));
    return await Promise.all(allFilesPaths.map((file) => {
        const filePath = path.join(cwd, file.path);
        return decoratorExtractor.getDecoratorsFromFile(filePath);
    }));
}
export async function scanClassesForDecorators(yamlBackend) {
    ...
    const classes = yamlBackend.classes || [];
    const overlappingYamlClasses = [];
    result.forEach((classInfo) => {
        if (classInfo.length < 1) {
            return;
        }
        if (Object.keys(classInfo[0]).length > 0) {
            const r = classes.find((c) => path.resolve(path.join(yamlBackend.path, c.path)) ===
                path.resolve(classInfo[0].path));
            const deployDecoratorFound = classInfo[0].decorators.find((d) => d.name === "GenezioDeploy");
