...
import fs from "fs";
import { isCallExpression, isIdentifier, isClassDeclaration, isObjectExpression, isObjectProperty, isStringLiteral, } from "@babel/types";
...
export class JsTsDecoratorExtractor extends DecoratorExtractor {
    fileFilter(cwd) {
        return (file) => {
            ...
            return ((file.extension === ".js" || file.extension === ".ts") &&
                !file.path.includes("node_modules") &&
                !file.path.includes(".git") &&
                !fs.lstatSync(folderPath).isDirectory());
        };
    }
    async getDecoratorsFromFile(file) {
                    ...
                    ClassDeclaration(path) {
                        if (path.node.decorators) {
                            ...
                            for (const decorator of path.node.decorators) {
                                ...
                                else if (isCallExpression(decorator.expression) &&
                                    isIdentifier(decorator.expression.callee)) {
                                    info.decorators = [
                                        ...(info.decorators ?? []),
                                        {
                                            name: decorator.expression.callee.name,
                                            arguments: JsTsDecoratorExtractor.parseArguments(decorator.expression.arguments),
                                        },
                                    ];
                                }
                            }
    ...
    async getDecoratorsTreeSitter(file) {
        const inputCode = fs.readFileSync(file, "utf8");
        const classes = [];
        const require = createRequire(import.meta.url);
    ...
    static parseArguments(args) {
        ...
        return args
            ...
            if (isObjectExpression(arg)) {
                return arg.properties.reduce((acc, curr) => {
                    if (isObjectProperty(curr) &&
