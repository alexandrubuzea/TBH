...
export async function watchPackage(language, projectName, frontend, sdkPath) {
    switch (language) {
        case Language.js:
        case Language.ts:
...
async function watchNodeModules(projectName, frontends, sdkPath) {
    ...
    return setInterval(async () => {
        for (const watchPath of watchPaths) {
            ...
            if (components[components.length - 1] === sdkName) {
                ...
                const res = compareSync(genezioSdkPath, watchPath, options);
                if (!res.same) {
...
async function writeSdkToNodeModules(projectName, frontends, originSdkPath) {
    const writeSdk = async (from, toFinal) => {
