...
class SdkGenerator {
    ...
    populateViewForMainClass(classDefinition, classConfiguration, view) {
    ...
    getModelFiles(modelClassesView) {
        const files = [];
     ...
     * @param toJson
     */
    addModelToModelViews(views, struct, fromJson, toJson) {
        if (!views[struct.path]) {
            views[struct.path] = {
                classes: [],
            };
        }
        if (!views[struct.path].classes.find((e) => e.name === struct.name)) {
            views[struct.path] = {
                classes: [
    ...
    generateFromJsonImplementationForClass(elem) {
        let implementation = "";
    ...
    generateToJsonImplementationForClass(elem) {
    ...
    castReturnTypeToPropertyType(node, variableName) {
        let implementation = "";
        switch (node.type) {
            ...
            case AstNodeType.PromiseType:
                implementation += this.castReturnTypeToPropertyType(node.generic, variableName);
                break;
    ...
    generateFromJsonImplementation(node, name) {
        let implementation = "";
        switch (node.type) {
            case AstNodeType.StringLiteral:
            ...
            case AstNodeType.IntegerLiteral:
                ...
                break;
            case AstNodeType.CustomNodeLiteral:
                implementation += `${node.rawValue}.fromJson(json['${name}'] as Map<String, dynamic>),`;
                break;
            case AstNodeType.ArrayType:
                implementation +=
                    castArrayRecursivelyInitial(node, `json['${name}']`) + ",";
                break;
            case AstNodeType.MapType:
                implementation +=
                    castMapRecursivelyInitial(node, `json['${name}']`) + ",";
                break;
            case AstNodeType.DateType:
    ...
    getReturnType(elem) {
        switch (elem.type) {
            ...
            case AstNodeType.VoidLiteral:
                return "Future<void>";
            case AstNodeType.PromiseType:
                return `Future<${getParamType(elem.generic)}>`;
            case AstNodeType.ArrayType:
                return `Future<List<${getParamType(elem.generic)}>>`;
            case AstNodeType.MapType:
                return `Future<Map<${getParamType(elem.genericKey)}, ${getParamType(elem.genericValue)}>>`;
            case AstNodeType.CustomNodeLiteral:
                return `Future<${elem.rawValue}>`;
            case AstNodeType.DateType:
