 ...
 */
export async function generateAst(input, plugins) {
    ...
    if (plugins) {
        pluginsImported = await Promise.all(plugins?.map(async (plugin) => {
            ...
            if (pluginSchema.safeParse(dynamicPlugin).success === false) {
                throw new UserError(`Plugin(${plugin}) is not a valid AST generator plugin. It must export a AstGenerator class and supportedExtensions array.`);
            }
    ...
    if (!plugin) {
        ...
        throw new UserError(`Class language(${extension}) not supported. Currently supporting: ${supportedExtensionsString}. You can delete the class from genezio.yaml`);
    }
