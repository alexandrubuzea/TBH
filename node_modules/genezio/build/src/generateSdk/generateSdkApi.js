 ...
 */
export async function sdkGeneratorApiHandler(language, classes, backendPath, packageName, packageVersion) {
    const inputs = generateAstInputs(classes || [], backendPath);
    ...
    for (const input of inputs) {
        ...
        const astGeneratorOutput = await generateAst(input, 
        ...
        if (!classConfiguration) {
            throw new UserError(`[Sdk Generator] Class configuration not found for ${input.class.path}`);
        }
    ...
    const sdkOutput = await generateSdk(sdkGeneratorInput, 
...
export function mapYamlClassToSdkClassConfiguration(classes, language, rootPath) {
...
export function generateAstInputs(classes, root) {
    const astGeneratorInputs = [];
