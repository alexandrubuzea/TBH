...
export class NodeJsLocalBundler {
    async bundle(input) {
        const selectedClass = input.projectConfiguration.classes.find((c) => input.configuration.name === c.name);
        if (!selectedClass) {
            throw new Error("Class not found");
        }
        const isClusterDeployment = input.projectConfiguration.cloudProvider === CloudProviderIdentifier.CLUSTER;
        await writeToFile(input.path, "local.mjs", isClusterDeployment ? localClusterWrapperCode : localWrapperCode, true);
        return {
            ...input,
            extra: {
                ...input.extra,
