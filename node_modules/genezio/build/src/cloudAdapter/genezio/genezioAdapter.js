...
export class GenezioCloudAdapter {
    async deploy(input, projectConfiguration, cloudAdapterOptions) {
        const stage = cloudAdapterOptions.stage || "";
        ...
        const promisesDeploy = input.map(async (element) => {
            ...
            if (element.unzippedBundleSize > BUNDLE_SIZE_LIMIT) {
                ...
                if (!dependenciesInfo || !allNonJsFilesPaths) {
                    throw new UserError(`Your class ${element.name} is too big: ${element.unzippedBundleSize} bytes. The maximum size is 250MB. Try to reduce the size of your class.`);
                }
                ...
                for (let i = 0; i < maxLength; i++) {
                    const formatedDep = allfilesSize.dependenciesSize[i]
                        ? allfilesSize.dependenciesSize[i]
                        : "";
                    const formatedNonJsFile = allfilesSize.filesSize[i]
...
Class ${element.name} is too big: ${(element.unzippedBundleSize / 1048576).toFixed(2)}MB. The maximum size is ${BUNDLE_SIZE_LIMIT / 1048576}MB. Try to reduce the size of your class.
            ...
            const presignedUrl = await getPresignedURL(projectConfiguration.region, "genezioDeploy.zip", projectConfiguration.name, element.name);
            const size = await getFileSize(element.archivePath);
            if (size > BUNDLE_SIZE_LIMIT) {
                throw new UserError(`Your class ${element.name} is too big: ${size} bytes. The maximum size is 250MB. Try to reduce the size of your class.`);
            }
...
function getClassFunctionUrl(baseUrl, cloudProvider, className) {
