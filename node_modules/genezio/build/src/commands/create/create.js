...
import { mergeContexts } from "yaml-transmute";
...
export async function createCommand(options) {
    ...
    switch (options.type) {
        case "fullstack": {
            ...
            await doAdaptiveLogAction("Installing template dependencies", async () => Promise.all([
                installTemplatePackages(backendTemplates[options.backend]?.pkgManager, path.join(projectPath, "server")),
                installTemplatePackages(frontendTemplates[options.frontend]?.pkgManager, path.join(projectPath, "client")),
            ]));
        ...
        case "backend": {
            ...
            // Install template packages
            await doAdaptiveLogAction("Installing template dependencies", async () => installTemplatePackages(backendTemplates[options.backend].pkgManager, projectPath));
            // Print success message
...
export function checkProjectName(projectName) {
    ...
    if (!projectNameRegex.test(projectName)) {
        throw new UserError("Project name must start with a letter and contain only letters, numbers and dashes");
    }
}
export function checkPathIsEmpty(projectPath) {
    if (nativeFs.existsSync(projectPath)) {
        ...
        for (const file of files) {
            ...
            if (!allowedFiles.includes(file)) {
                throw new UserError(`A folder named '${projectPath}' already exists. You can't create a project in a non-empty folder.`);
            }
        }
    }
}
 ...
 */
async function createFullstackProject(fs, fullstackProjectOpts) {
    const backendProjectInfo = {
        name: fullstackProjectOpts.name,
        region: fullstackProjectOpts.region,
        type: "backend",
        templateId: fullstackProjectOpts.backend,
    };
    const frontendProjectInfo = {
        name: fullstackProjectOpts.name,
        region: fullstackProjectOpts.region,
        type: "frontend",
 ...
 * @returns A promise that resolves when the project creation is complete.
 */
async function createProject(fs, projectInfo, projectPath = "/") {
...
async function createWorkspaceYaml(fs, fullstackProjectOpts, backendPath, frontendPath) {
    const backendConfigReader = new YamlConfigurationIOController(path.join(backendPath, "genezio.yaml"), {}, fs);
    ...
    if (backendConfiguration.backend) {
        backendConfiguration.backend.path = path.relative("/", path.join(backendPath, backendConfiguration.backend.path));
    }
    const frontendConfigReader = new YamlConfigurationIOController(path.join(frontendPath, "genezio.yaml"), {}, fs);
    ...
    if (frontendTemplates[fullstackProjectOpts.frontend] === undefined) {
        ...
        if (frontendConfiguration.frontend) {
            if (!Array.isArray(frontendConfiguration.frontend)) {
                frontendConfiguration.frontend.path = path.relative("/", path.join(frontendPath, frontendConfiguration.frontend.path));
            }
            else {
    ...
    const contexts = [backendConfigReader.ctx, frontendConfigReader.ctx].filter((c) => c !== undefined);
 ...
 */
async function replacePlaceholders(fs, replacePath, placeholders) {
    const fromStats = fs.statSync(replacePath);
    if (fromStats.isDirectory()) {
        const files = fs.readdirSync(replacePath);
        for (const file of files) {
            replacePlaceholders(fs, path.join(replacePath, file), placeholders);
        }
    }
    else {
        const fileContent = fs.readFileSync(replacePath, "utf8");
        if (!fileContent.includes("(•◡•)")) {
            return;
        }
        const newFileContent = Object.entries(placeholders).reduce((acc, [placeholder, value]) => acc.replaceAll(placeholder, value), fileContent);
        fs.writeFileSync(replacePath, newFileContent);
    }
}
function copyRecursiveToNativeFs(fromFs, from, to, options) {
    const fromStats = fromFs.statSync(from);
    if (fromStats.isDirectory()) {
        nativeFs.mkdirSync(to, { recursive: true });
        const files = fromFs.readdirSync(from);
        for (const file of files) {
            if (options?.keepDotGit &&
                file === ".git" &&
 ...
 */
async function installTemplatePackages(packageManager, path) {
    try {
...
const SUCCESSFULL_CREATE_MONOREPO = (projectPath, projectName) => `Project initialized in ${projectPath}.
    ...
    For ${colors.green("testing")} locally, run:
        ...
        genezio local
`;
const SUCCESSFULL_CREATE_MULTIREPO = (projectPath, projectName) => `Project initialized in ${projectPath}. Now run:
    ...
    For ${colors.green("testing")} locally, run:
      ...
      Terminal 2 (start the frontend): 
        ...
        genezio local
`;
const SUCCESSFULL_CREATE_BACKEND = (projectPath, projectName) => `Project initialized in ${projectPath}. Now run:
    ...
    For ${colors.green("testing")} locally, run:
        ...
        genezio local
`;
const SUCCESSFULL_CREATE_NO_FRONTEND = (projectPath) => `Project initialized in ${projectPath}.
