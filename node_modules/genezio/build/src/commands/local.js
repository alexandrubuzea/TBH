...
import { isDependencyVersionCompatible, checkExperimentalDecorators, } from "../utils/jsProjectChecker.js";
...
export async function prepareLocalBackendEnvironment(yamlProjectConfiguration, options) {
    try {
        ...
        const sdk = await sdkGeneratorApiHandler(sdkLanguage, mapYamlClassToSdkClassConfiguration(backend.classes, backend.language.name, backend.path), backend.path, 
...
export async function startLocalEnvironment(options) {
    ...
    await Promise.all([
        startBackendWatcher(yamlProjectConfiguration.backend, options, sdkSynchronizer).catch((e) => {
 ...
 */
async function startFrontends(frontendConfiguration, sdkSynchronizer) {
    ...
    await Promise.all(frontendConfiguration.map(async (frontend) => {
        await runFrontendStartScript(frontend.scripts?.start, frontend.path).catch((e) => log.error(new Error(`Failed to start frontend located in \`${frontend.path}\`: ${e.message}`)));
    }));
}
/**
...
async function startBackendWatcher(backendConfiguration, options, sdkSynchronizer) {
    ...
    while (true) {
        ...
        const classProcessSpawnPromise = prepareLocalBackendEnvironment(yamlProjectConfiguration, options);
        ...
        if (promiseRes.restartEnvironment === true) {
            ...
            promiseRes.spawnOutput.processForClasses.forEach((classProcess) => {
        ...
        const processForClasses = promiseRes.spawnOutput.processForClasses;
        ...
        const watcherTimeout = await handleSdk(yamlProjectConfiguration.name, yamlProjectConfiguration.frontend, sdk, options);
 ...
 */
async function startProcesses(projectConfiguration, sdk, options) {
    const classes = projectConfiguration.classes;
    const processForClasses = new Map();
    // Bundle each class and start a new process for it
    const bundlersOutputPromise = classes.map(async (classInfo) => {
        ...
        const astClass = sdk.sdkGeneratorInput.classesInfo.find((c) => c.classConfiguration.path === classInfo.path);
    ...
    for (const bundlerOutput of bundlersOutput) {
        ...
        await startClassProcess(extra.startingCommand, extra.commandParameters ? extra.commandParameters : [], bundlerOutput.configuration.name, processForClasses, envVars, projectConfiguration.workspace?.backend);
    }
...
async function startServerHttp(port, astSummary, projectName, processForClasses) {
    const app = express();
...
function startCronJobs(projectConfiguration, processForClasses) {
    const cronHandlers = [];
        ...
        for (const method of methods) {
            if (method.type === TriggerType.cron && method.cronString) {
                ...
                cronHandler.cronObject = cron.schedule(cronHandler.cronString, () => {
                    ...
                    void communicateWithProcess(cronHandler.process, cronHandler.className, reqToFunction, processForClasses);
                });
...
async function clearAllResources(server, processForClasses, crons) {
...
async function startClassProcess(startingCommand, parameters, className, processForClasses, envVars = {}, cwd) {
...
async function communicateWithProcess(localProcess, className, data, processForClasses) {
    try {
        ...
        if (error instanceof AxiosError &&
            ...
            await startClassProcess(localProcess.startingCommand, localProcess.parameters, className, processForClasses, localProcess.envVars);
        }
        throw error;
    }
}
