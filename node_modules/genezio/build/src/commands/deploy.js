...
import { ENVIRONMENT, DASHBOARD_URL, RECOMMENTDED_GENEZIO_TYPES_VERSION_RANGE, REQUIRED_GENEZIO_TYPES_VERSION_RANGE, } from "../constants.js";
...
import { fileExists, createTemporaryFolder, zipDirectory, isDirectoryEmpty, directoryContainsIndexHtmlFiles, directoryContainsHtmlFiles, deleteFolder, getBundleFolderSizeLimit, readEnvironmentVariablesFile, zipFile, } from "../utils/file.js";
...
import { checkExperimentalDecorators, isDependencyVersionCompatible, } from "../utils/jsProjectChecker.js";
...
import configIOController from "../yamlProjectConfiguration/v2.js";
...
export async function deployCommand(options) {
    ...
    if (configuration.backend?.language.name === Language.ts ||
    ...
    backend: if (configuration.backend && !options.frontend) {
        if (configuration.backend.classes?.length === 0) {
            log.error("No classes were found in your genezio.yaml. Add some to be able to deploy your backend.");
            break backend;
        }
        ...
        if (ENVIRONMENT === "dev" &&
            ...
            yamlConfig.backend.cloudProvider = await performCloudProviderABTesting(configuration.name, configuration.region, configuration.backend.cloudProvider);
        ...
        deployClassesResult = await deployClasses(configuration, options).catch(async (error) => {
            await GenezioTelemetry.sendEvent({
    ...
    if (configuration.frontend && !options.backend) {
        ...
        for (const [index, frontend] of frontends.entries()) {
            try {
                await doAdaptiveLogAction(`Running frontend ${index + 1} deploy script`, async () => {
                    return await runScript(frontend.scripts?.deploy, frontend.path || process.cwd());
                });
            }
            catch (error) {
                await GenezioTelemetry.sendEvent({
            ...
            const frontendUrl = await deployFrontend(configuration.name, configuration.region, frontend, index, options).catch(async (error) => {
    ...
    if (deployClassesResult) {
        log.info(colors.cyan(`App Dashboard URL: ${DASHBOARD_URL}/project/${deployClassesResult.projectId}/${deployClassesResult.projectEnvId}`));
    }
...
export async function deployClasses(configuration, options) {
    ...
    const sdkResponse = await sdkGeneratorApiHandler(sdkLanguage, mapYamlClassToSdkClassConfiguration(backend.classes, backend.language.name, backend.path), backend.path, 
    /* packageName= */ `@genezio-sdk/${configuration.name}`).catch((error) => {
    ...
    if (classesWithNoMethods.length) {
        ...
        throw new UserError(`Unable to deploy classes [${errorClasses}] as they do not have any methods.`);
    }
    ...
    const bundlerResult = projectConfiguration.classes.map(async (element) => {
        ...
        const unzippedBundleSize = await getBundleFolderSizeLimit(output.path);
    ...
    const cloudAdapter = getCloudAdapter(configuration.backend?.cloudProvider || CloudProviderIdentifier.GENEZIO);
    ...
    if (sdkResponse.files.length <= 0) {
        ...
        log.info(colors.cyan("Your backend code was deployed and the SDK was successfully generated"));
    }
        ...
        if (options.env) {
            ...
            if (!(await fileExists(envFile))) {
                ...
                await setEnvironmentVariables(projectId, projectEnv.id, envVars)
                    .then(async () => {
                    debugLogger.debug(`Environment variables from ${envFile} uploaded to project ${projectId}`);
                    ...
                    log.error(`Try to set the environment variables using the dashboard ${colors.cyan(DASHBOARD_URL)}`);
                    await GenezioTelemetry.sendEvent({
                        eventType: TelemetryEventTypes.GENEZIO_DEPLOY_ERROR,
                        errorTrace: error.toString(),
                    });
            ...
            if (await fileExists(envFile)) {
                ...
                const missingEnvVars = envVars.filter((envVar) => !remoteEnvVars.find((remoteEnvVar) => remoteEnvVar.name === envVar.name));
                ...
                if (missingEnvVars.length > 0) {
                    ...
                    log.info("");
                    log.info(`${colors.yellow("Go to the dashboard ")}${colors.cyan(DASHBOARD_URL)} ${colors.yellow("to set your environment variables or run ")} ${colors.cyan(`genezio deploy --env ${relativeEnvFilePath}`)}`);
                    log.info("");
                }
            }
        }
...
export async function deployFrontend(name, region, frontend, index, options) {
    const stage = options.stage || "";
    if (!frontend.publish) {
        log.info(`Skipping frontend deployment for \`${frontend.path}\` because it has no publish folder in the YAML configuration. Check https://genezio.com/docs/project-structure/genezio-configuration-file for more details.`);
        return;
    }
    try {
    ...
    if (!(await fileExists(frontendPath))) {
        throw new UserError(`The publish folder ${colors.cyan(`${frontendPath}`)} does not exist. Please run the build command first or add a \`deploy\` script in the genezio.yaml file.`);
    }
    // check if the publish folder is empty
    if (await isDirectoryEmpty(frontendPath)) {
        throw new UserError(`The publish folder ${colors.cyan(`${frontendPath}`)} is empty. Please run the build command first or add a \`deploy\` script in the genezio.yaml file.`);
    }
    ...
    if (!options.subdomain && !frontend.subdomain) {
        log.info("No subdomain specified in the genezio.yaml configuration file or as an option flag. We will provide a random one for you.");
...
async function handleSdk(configuration, result, sdkResponse, options) {
    const frontends = configuration.frontend;
    let sdkLanguage = Language.ts;
    let sdkPath, frontendPath;
...
export async function performCloudProviderABTesting(projectName, projectRegion, projectCloudProvider) {
    ...
    if (!isAlreadyDeployed && projectCloudProvider === CloudProviderIdentifier.GENEZIO) {
        ...
        if (randomCloudProvider !== CloudProviderIdentifier.GENEZIO) {
            debugLogger.debug("You've been visited by the AB testing fairy! 🧚‍♂️ Your cloud provider is now set to", randomCloudProvider);
            debugLogger.debug("To disable AB testing, run `DISABLE_AB_TESTING=true genezio deploy`.");
            randomCloudProvider;
        }
